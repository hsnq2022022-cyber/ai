...
let obBuilding=false;
async function obBuildAgent(){
const s=ws();const st=s.settings;
if(st.obBuilt||obBuilding)return;
obBuilding=true;
try{await obBuildAgentInner(s,st);}finally{obBuilding=false;}}

async function obBuildAgentInner(s,st){
...
s.widgets.push(w);
st.obAgentId=agent.id;st.obWidgetId=w.id;
st.onboardingAgentId=agent.id;st.onboardingWidgetId=w.id;
save();persist();
if(isRemote()){
try{await sbUpsert('agents',agent);await syncWidgetToServer(w);}
catch(e){
s.agents=s.agents.filter(function(a){return a.id!==agent.id});
s.widgets=s.widgets.filter(function(x){return x.id!==w.id});
st.obAgentId=null;st.obWidgetId=null;st.onboardingAgentId=null;st.onboardingWidgetId=null;
save();persist();
throw new Error('فشل الحفظ في قاعدة البيانات: '+e.message);
}}
st.obBuilt=true;save();persist();}
