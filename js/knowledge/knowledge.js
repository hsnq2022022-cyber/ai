...
const resp=await fetch(apiBase()+'/kb-process',{method:'POST',headers:Object.assign({'Content-Type':'application/json','api-key':cfg.key},await authHeaders()),body:JSON.stringify({doc_id:doc.id,path:path,name:file.name,agent_id:agentId||null,workspace_id:db.ws.__wid})});
...
try{
if(isRemote()&&cfg&&apiBase()&&db.ws&&db.ws.__wid){
const resp=await fetch(apiBase()+'/kb-ingest',{method:'POST',headers:Object.assign({'Content-Type':'application/json','api-key':cfg.key},await authHeaders()),body:JSON.stringify({doc_id:doc.id,text:doc.content,agent_id:doc.agentId||null,workspace_id:db.ws.__wid})});
const j=await resp.json().catch(function(){return null});
...
try{
if(isRemote()&&cfg&&apiBase()&&db.ws&&db.ws.__wid){
const resp=await fetch(apiBase()+'/kb-process',{method:'POST',headers:Object.assign({'Content-Type':'application/json','api-key':cfg.key},await authHeaders()),body:JSON.stringify({doc_id:doc.id,path:doc.storagePath,name:doc.name,agent_id:doc.agentId||null,workspace_id:db.ws.__wid})});
const j=await resp.json().catch(function(){return null});
