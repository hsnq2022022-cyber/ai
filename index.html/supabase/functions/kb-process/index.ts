...
const dr0=await admin.from("knowledge_docs").select("workspace_id").eq("id",doc_id).maybeSingle();
if(!dr0.data)return json({error:"not_found"},404);
const wsChk=await admin.from("workspaces").select("id").eq("id",dr0.data.workspace_id).eq("owner_id",ownerId).maybeSingle();
if(!wsChk.data)return json({error:"forbidden",message:"لا تملك صلاحية على مساحة العمل هذه."},403);
if(!path.startsWith(dr0.data.workspace_id+"/"))return json({error:"forbidden"},403);
